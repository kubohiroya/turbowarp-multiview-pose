...
(function (Scratch) {
  	...
  	blocks: [
  		{
  			...
  			"blockType": "COMMAND",
  			"text": "prepare offer QR for peer [PEER]",
  			"description": "Creates a WebRTC offer and prepares one or more QR courier parts.",
  			"arguments": { "PEER": {
  				"type": "STRING",
  		...
  		},
  		{
  			...
  			"blockType": "COMMAND",
  			"text": "show offer QR part [INDEX] on this sprite",
  			"description": "Shows the selected one-based offer QR part using a temporary sprite skin.",
  			"arguments": { "INDEX": {
  				"type": "NUMBER",
  				"defaultValue": 1
  			} }
  		},
  		{
  			...
  			"blockType": "REPORTER",
  			"text": "offer QR part count",
  			"description": "Returns the number of prepared offer QR parts, or zero when none is prepared.",
  			"arguments": {}
  		},
  		{
  			"opcode": "offerQrCurrentPart",
  			"blockType": "REPORTER",
  			"text": "current offer QR part",
  			"description": "Returns the one-based part currently selected for display, or zero when none is prepared.",
  			"arguments": {}
  		},
  		{
  			"opcode": "showNextOfferQrPart",
  			"blockType": "COMMAND",
  			"text": "show next offer QR part on this sprite",
  			"description": "Shows the next part and wraps from the last part to the first.",
  			"arguments": {}
  		},
  		{
  			"opcode": "createAndShowOfferQr",
  			"blockType": "COMMAND",
  			"text": "show offer QR for peer [PEER] on this sprite",
  			"description": "Creates an offer and shows its first QR courier part on this sprite.",
  			"arguments": { "PEER": {
  				"type": "STRING",
  				"defaultValue": "camera-1"
  			} }
  		},
  		{
  			...
  			"blockType": "COMMAND",
  			"text": "end offer QR display",
  			"description": "Restores original sprite skins and discards all temporary offer QR data.",
  			"arguments": {}
  		},
  		{
  			"opcode": "offerQrState",
  			"blockType": "REPORTER",
  			"text": "offer QR state",
  			"description": "Returns idle, generating-offer, rendering, displayed, or error.",
  			"arguments": {}
  		},
  		{
  			"opcode": "offerQrError",
  			"blockType": "REPORTER",
  			"text": "offer QR error",
  			"description": "Returns the latest QR pairing error message.",
  			"arguments": {}
  		}
  	]
  };
  ...
  var featureFlags = Object.freeze({ qrCourierPairing: overrides?.qrCourierPairing === true });
  //#endregion
  //#region config/qr-config.ts
  var configuredLevel = globalThis.__TWMP_QR_CONFIG__?.errorCorrectionLevel;
  ...
  var QR_COURIER_PROTOCOL = "twmp-qr/1";
  var MAX_CHUNK_LENGTH = 4096;
  async function createQrCourierParts(message, options) {
  	...
  	for (;;) {
  		...
  		if (chunkLength < 1) throw new Error("QR encoder capacity is too small for the part envelope.");
  		const required = Math.ceil(message.length / chunkLength);
  		if (required > 64) throw new Error(`QR encoder capacity exceeded: ${required} parts exceeds 64.`);
  ...
  function serializeQrCourierPart(part) {
  	validatePart(part);
  ...
  function validatePart(part) {
  	if (part.protocol !== "twmp-qr/1") throw new Error("Unsupported QR part protocol.");
  	...
  	if (part.kind !== "offer" && part.kind !== "answer") throw new Error("Invalid QR message kind.");
  	requireTimestamp(part.createdAt);
  	if (!Number.isInteger(part.partCount) || part.partCount < 1 || part.partCount > 64) throw new Error("Invalid QR part count.");
  	if (!Number.isInteger(part.partIndex) || part.partIndex < 0 || part.partIndex >= part.partCount) throw new Error("Invalid QR part index.");
  	if (!Number.isInteger(part.messageLength) || part.messageLength < 1 || part.messageLength > 131072) throw new Error("Invalid QR message length.");
  	if (!/^[A-Za-z0-9_-]{43}$/u.test(part.messageHash)) throw new Error("Invalid QR message hash.");
  	if (typeof part.payload !== "string" || part.payload.length > 4096) throw new Error("Invalid QR part payload.");
  }
  function requirePairingCode(value) {
  	if (typeof value !== "string" || value.length < 1) throw new Error("Offer pairing code is empty.");
  	if (value.length > 131072) throw new Error("Offer pairing code is too large.");
  	if (!/^[\x20-\x7E]+$/u.test(value)) throw new Error("Offer pairing code must be printable ASCII.");
  }
  ...
  function requireTimestamp(value) {
  	if (!Number.isSafeInteger(value) || value < 0) throw new Error("Invalid QR creation timestamp.");
  	return value;
  }
  ...
  var TemporarySpriteSkinManager = class {
  	...
  	validateTarget(target) {
  		if (!target || target.isStage) throw new Error("Offer QR must be displayed on a sprite target.");
  		if (target.isOriginal === false) throw new Error("Offer QR display does not support sprite clones.");
  		if (!Number.isInteger(target.drawableID) || Number(target.drawableID) < 0) throw new Error("Offer QR sprite target has no valid drawable.");
  		return target;
  	}
  	show(targetValue, svg) {
  		...
  		if (!Number.isInteger(temporarySkinId) || temporarySkinId < 0) throw new Error("Renderer failed to create a temporary QR skin.");
  		try {
  			renderer.updateDrawableSkinId(drawableId, temporarySkinId);
  		} catch (error) {
  			renderer.destroySkin(temporarySkinId);
  			throw new Error("Renderer failed to display the temporary QR skin.", { cause: error });
  		}
  ...
  function requireRenderer(renderer) {
  	if (!renderer || typeof renderer.createSVGSkin !== "function" || typeof renderer.destroySkin !== "function" || typeof renderer.updateDrawableSkinId !== "function") throw new Error("TurboWarp renderer does not provide temporary SVG skin APIs.");
  	return renderer;
  }
  function currentSkinId(renderer, drawableId) {
  	const skinId = renderer._allDrawables?.[drawableId]?._skin?._id;
  	if (!Number.isInteger(skinId) || Number(skinId) < 0) throw new Error("Renderer could not determine the sprite original skin.");
  	return Number(skinId);
  }
  //#endregion
  //#region src/webrtc-capability.ts
  var WEBRTC_CAPABILITY_KEY = "kubohiroyaWebRtcCapability";
  function requireWebRtcOfferCapability(runtime) {
  	const candidate = runtime[WEBRTC_CAPABILITY_KEY];
  	if (!isRecord(candidate)) throw new Error("TurboWarp WebRTC is not loaded.");
  	if (typeof candidate.requireVersion !== "function" || typeof candidate.createOffer !== "function" || typeof candidate.getOffer !== "function") throw new Error("TurboWarp WebRTC does not provide the offer runtime API.");
  	try {
  		candidate.requireVersion(2);
  	} catch (error) {
  		throw new Error(`TurboWarp WebRTC runtime capability v2 is required.`, { cause: error });
  	}
  	return candidate;
  }
  ...
  var MultiviewPoseExtension = class {
  	constructor(options = {}) {
  		...
  		this.lastError = "";
  		this.operation = 0;
  		this.stopListener = () => this.endOfferQrDisplay();
  	...
  	async prepareOfferQr(args) {
  		this.requireEnabled();
  		if (this.state === "generating-offer" || this.state === "rendering") throw new Error("An offer QR operation is already running.");
  		...
  		try {
  			...
  			if (!code) throw new Error("TurboWarp WebRTC did not return an offer pairing code.");
  			...
  			this.session = {
  				peer,
  				texts: courier.texts,
  				svgs: courier.texts.map((text) => createQrSvg(text, this.errorCorrectionLevel)),
  				currentIndex: 0
  			};
  	...
  	showOfferQrPart(args, util) {
  		this.requireEnabled();
  		const session = this.requireSession();
  		const index = Scratch.Cast.toNumber(args.INDEX);
  		if (!Number.isInteger(index) || index < 1 || index > session.svgs.length) throw new Error(`Offer QR part index must be between 1 and ${session.svgs.length}.`);
  	...
  	showNextOfferQrPart(_args, util) {
  		this.requireEnabled();
  		const session = this.requireSession();
  	...
  	async createAndShowOfferQr(args, util) {
  		...
  		if (this.runtime.targets && !this.runtime.targets.includes(target)) {
  			...
  			throw new Error("Offer QR sprite target was removed while the offer was being created.");
  		}
  	...
  	requireEnabled() {
  		if (!this.enabled) throw new Error("QR courier pairing is disabled. Enable it before the project starts.");
  	}
  	requireSession() {
