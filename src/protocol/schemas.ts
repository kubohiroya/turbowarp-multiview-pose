...
const timestampUs = Type.Integer({
  minimum: 0,
  maximum: Number.MAX_SAFE_INTEGER,
});
const score = Type.Number({ minimum: 0, maximum: 1 });
const boundedNumber = Type.Number({
  minimum: -1_000_000,
  maximum: 1_000_000,
});
const utcDateTime = Type.String({
...
const object = <T extends Record<string, TSchema>>(
  properties: T,
  options: { $id?: string } = {},
...
export const SessionPolicySchema = object(
  {
    ...
    poseChannelHighWaterBytes: Type.Integer({
      minimum: 1024,
      maximum: 16_777_216,
    }),
  },
...
export const CameraCalibrationSchema = object(
  {
    ...
    distortionCoefficients: Type.Array(boundedNumber, {
      minItems: 0,
      maxItems: 14,
    }),
    worldFromCameraMatrix: Type.Tuple(
      Array.from({ length: 16 }, () => boundedNumber),
    ),
...
export const PoseFrame2DSchema = object(
  {
    ...
    calibrationId: identifier,
    persons: Type.Array(
      object({ trackingId: identifier, score, keypoints: keypoints2d }),
      { maxItems: 6 },
    ),
  },
  {
...
export const PoseFrame3DSchema = object(
  {
    ...
    persons: Type.Array(
      object({
        personId: identifier,
        score,
        cameraIds: Type.Array(identifier, {
          minItems: 2,
          maxItems: 16,
          uniqueItems: true,
        }),
        meanReprojectionErrorPx: Type.Number({
          minimum: 0,
          maximum: 100_000,
        }),
