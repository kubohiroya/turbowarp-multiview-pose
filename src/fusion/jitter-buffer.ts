...
export class MultiCameraJitterBuffer {
  ...
  public constructor(private options: JitterBufferOptions) {}
...
function keypointsById(
  person: PoseFrame2DPersonV1,
): Map<Coco17KeypointId, PoseFrame2DKeypointV1> {
  return new Map(person.keypoints.map((keypoint) => [keypoint.id, keypoint]));
}

function isVisible(
  keypoint: PoseFrame2DKeypointV1 | undefined,
  minimumScore: number,
): boolean {
